...
3. Include ALL competitor moves from: {competitors}. This is a KEY section of the report.
...
<h2>🏢 Competitor Intelligence — Global Market Moves</h2>
Group competitor news BY COMPETITOR. For each competitor that has news, create a <h3> with the competitor name.
Cover these competitors: {competitors}.
Include: contract awards, project wins, earnings, expansions, M&A, new products, leadership changes.
If a competitor has no news, DO NOT create its <h3>. If NO competitors have news at all, write: No significant competitor activity detected this period.

Then:
<h2>📰 Nexans Worldwide</h2> (ALWAYS INCLUDE. Cover Nexans Group, all subsidiaries and brands. If none, write: No Nexans mentions this period.)
<h2>🎯 Commercial Opportunities</h2> (Only if concrete: awarded contracts, tenders, announced projects with investment figures)
...
Act as Global Marketing Director. Provide 1-2 paragraphs on what actions the commercial teams in LATAM (Colombia, Peru, Chile, Ecuador, Brazil) should take based on the global signals observed. Be SPECIFIC — mention competitor names, project names, and regions.
...
- NO agregues saludos, títulos extra fuera del HTML ni explicaciones adicionales.";
        ...
        for (int attempt = 0; attempt <= maxRetries; attempt++)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Gemini Summarizer returned no candidates");
                    return string.Empty;
                }
                ...
                {
                    _logger.LogWarning("Gemini Summarizer returned empty content/parts");
                    return string.Empty;
                }
